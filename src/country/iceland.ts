    ...
    async  getSlotsAvailable() {
        try {
            ...
            this.page.on('response', async (response) => {
                ...
                if (url.includes('https://lift-api.vfsglobal.com/appointment/CheckIsSlotAvailable')) {
            ...
            await this.delay(2000); // Let page stabilize
            ...
            console.log("Looking for Iceland Visa Application Center, London option...");
            ...
            const optionSelected = await this.page.evaluate(() => {
                ...
                // Fallback: look for just "Iceland"
                for (const option of options) {
                    const text = option.textContent?.trim() || '';
                    if (text.toLowerCase().includes('iceland')) {
                        console.log("Found Iceland option (fallback):", text);
                        (option as HTMLElement).click();
                        return true;
                    }
                }
